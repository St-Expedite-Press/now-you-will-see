...
USAGE — end a task (most common invocation, called explicitly by user):
    python tools/task_logger.py end \\
...
USAGE — harvest most recent Claude Code session into a task:
    python tools/task_logger.py harvest
    python tools/task_logger.py harvest --session <session-id>
    python tools/task_logger.py harvest --session <session-id> --subagent <agent-id>

USAGE — list recorded tasks:
    python tools/task_logger.py list

USAGE — build dataset from labeled tasks:
    python tools/task_logger.py build
...
def cmd_end(args: argparse.Namespace) -> int:
    ...
    print(f"Then: python tools/task_logger.py build")
    return 0
...
def cmd_build(args: argparse.Namespace) -> int:
    ...
    print("  python tools/task_logger.py build --no-metadata")
    return 0
