...
export default function IngestProjectPicker() {
  ...
  return (
    <main className="min-h-screen bg-[#0b0b0a] px-5 py-6 text-[#ebe7de]">
      <div className="mx-auto max-w-5xl">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-[#343129] pb-6">
          <div>
            <Link to="/" className="inline-flex items-center gap-2 text-xs text-[#8f887b] hover:text-[#ebe7de]">
              <ArrowLeft className="h-3.5 w-3.5" />
              Studio entry
            </Link>
            <h1 className="mt-3 text-2xl font-medium text-[#ebe7de]">Ingest Documents</h1>
          </div>
          <Link
            to="/projects/new"
        ...
        <section className="py-8">
          {loading && <p className="text-sm text-[#8f887b]">Loading projects...</p>}
          {error && (
            ...
            <div className="border-y border-[#343129]">
              {workspace.projects.map((project) => (
                <button
                  key={project.id}
                  type="button"
                  onClick={() => navigate(`/projects/${project.id}/ingest`)}
